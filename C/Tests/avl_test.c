...
#include "avl_test.h"
#include "../COOP/UnitTestsInfra.h"
#include "../COOP/COOP.h"
#include <stdlib.h>
#include <math.h>
#include <limits.h>

static int isBSTHelper(AVLTreeNode* node, int min, int max) {
    if (node == NULL) {
        return 1;
    }
...
static int isAVLHelper(AVLTreeNode* node) {
    if (node == NULL) {
        return 1;
    }
    int leftHeight = Tree_node_height(node->left);
    int rightHeight = Tree_node_height(node->right);
...
static int AVL_IsValid(AVLTree* tree) {
